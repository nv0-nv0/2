...
(async()=>{
  try {
    ...
    document.getElementById('recentScans').innerHTML = renderList(diag.recentScans || [], '<div class="muted">스캔 없음</div>', (item) => `<div class="result-card"><strong>${escapeHtml(item.target)}</strong><div class="muted">${escapeHtml(item.riskScore)}점 · ${escapeHtml(item.riskLevel)}</div></div>`);
