...
function fallbackOffers(){
  return [
    {code:'Report',group:'one_time',title:'상세 리포트',price:69000,period:'1회',summary:'무료 진단 결과를 근거와 우선순위가 있는 리포트로 확장합니다.',targetCustomer:'무엇부터 고칠지 판단해야 하는 운영자',deliverables:['위험도 해설','우선 조치 목록','재점검 기준'],operations:['결제 전 제공 범위 확인','디지털 산출물 제공 기준 확인'],benefits:['개선 순서 확인','내부 공유 가능'],cta:'상세 리포트 신청',referencePrice:100000,valuePackWorth:207000},
...
function renderSmartAdvice(intelligence, orchestration){
  ...
  smartPlanAdvice.innerHTML=`<article class="card stack smart-advice-card"><div class="meta-row"><span class="pill brand">스마트 추천</span><span class="pill ${intelligence?.riskBand?.tone==='success'?'green':'gold'}">${escapeHtml(orchestration?.stage?.label||intelligence?.riskBand?.label||'추천')}</span></div><h2>${escapeHtml(next.title||intelligence?.headline||'다음 행동을 정리했습니다.')}</h2><p>${escapeHtml(next.description||intelligence?.reason||'진단 결과에 맞춰 다음 선택지를 정리했습니다.')}</p><div class="smart-action-grid">${actions.map(item=>`<span>${escapeHtml(item)}</span>`).join('')}</div><div class="topnav"><a class="btn primary" href="${escapeHtml(path)}">${escapeHtml(next.cta||intelligence?.primaryCta||'추천 흐름 보기')}</a><a class="btn secondary" href="/portal">내 사이트 관리</a></div><small class="muted">${escapeHtml(orchestration?.caveat||intelligence?.caveat||'법률 자문이나 성과 보장을 대신하지 않습니다.')}</small></article>`;
}
