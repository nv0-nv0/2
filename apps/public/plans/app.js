...
function renderSmartAdvice(intelligence, orchestration){
  ...
  smartPlanAdvice.innerHTML=`<article class="card stack smart-advice-card"><div class="meta-row"><span class="pill brand">스마트 추천</span><span class="pill ${intelligence?.riskBand?.tone==='success'?'green':'gold'}">${escapeHtml(orchestration?.stage?.label||intelligence?.riskBand?.label||'추천')}</span></div><h2>${escapeHtml(next.title||intelligence?.headline||'다음 행동을 정리했습니다.')}</h2><p>${escapeHtml(next.description||intelligence?.reason||'무료진단 결과의 확인 범위와 다음 선택지를 정리했습니다.')}</p><div class="smart-action-grid">${actions.map(item=>`<span>${escapeHtml(item)}</span>`).join('')}</div><div class="topnav"><a class="btn primary" href="${escapeHtml(path)}">${escapeHtml(next.cta||intelligence?.primaryCta||'추천 흐름 보기')}</a><a class="btn secondary" href="/portal">내 사이트 관리</a></div><small class="muted">${escapeHtml(orchestration?.caveat||intelligence?.caveat||'법률 자문이나 성과 보장을 대신하지 않습니다.')}</small></article>`;
}

(async()=>{try{const saved=getSavedScan();const qs=new URLSearchParams(location.search);const siteId=qs.get('siteId')||saved?.siteId||'';const riskScore=Number(qs.get('riskScore')||saved?.riskScore||0);const [productsRes,plansRes]=await Promise.all([fetch(`/api/public/products?${qs.toString()}`),fetch(`/api/public/plans?${qs.toString()}`)]);const products=await productsRes.json();const plans=await plansRes.json();if(!products.ok)throw new Error(products.error||'상품 정보를 가져오지 못했습니다.');const offers=plans.smartOffers||products.offers||[];const intelligence=plans.intelligence||products.intelligence||{};const recommended=plans.recommendedPlan||intelligence.recommendedPlan||'Report';renderSmartAdvice(intelligence,plans.orchestration||products.orchestration);state.textContent=`선택 가능한 상품 ${offers.length}개를 확인할 수 있습니다${riskScore?` · 최근 진단 점수 ${riskScore}점 기준 추천 포함`:''}.`;oneTimeCards.innerHTML=offers.filter(o=>o.group==='one_time').map(o=>card(o,recommended===o.code,siteId)).join('');subscriptionCards.innerHTML=offers.filter(o=>o.group==='subscription').map(o=>card(o,recommended===o.code,siteId)).join('');certCards.innerHTML=offers.filter(o=>['annual','b2b'].includes(o.group)).map(o=>card(o,recommended===o.code,siteId)).join('');comparisonRows.innerHTML=offers.map(o=>`<tr><td><strong>${escapeHtml(o.title)}</strong><br><span class="muted">${escapeHtml(groupLabel(o))}</span></td><td class="price-cell">${priceLabel(o)}</td><td>${escapeHtml(o.smartReason||o.targetCustomer||'-')}</td><td>${roleLabel(o)}</td></tr>`).join('');}catch(error){const offers=fallbackOffers();state.textContent=`상품 정보를 불러오지 못했습니다. 기본 요금표를 표시합니다: ${error.message}`;oneTimeCards.innerHTML=offers.filter(o=>o.group==='one_time').map(o=>card(o,o.code==='Report','')).join('');subscriptionCards.innerHTML=offers.filter(o=>o.group==='subscription').map(o=>card(o,false,'')).join('');certCards.innerHTML='<div class="card muted">조직용 상품은 고객지원으로 문의해 주세요.</div>';comparisonRows.innerHTML=offers.map(o=>`<tr><td><strong>${escapeHtml(o.title)}</strong><br><span class="muted">${escapeHtml(groupLabel(o))}</span></td><td class="price-cell">${priceLabel(o)}</td><td>${escapeHtml(o.targetCustomer||'-')}</td><td>${roleLabel(o)}</td></tr>`).join('');}})();
