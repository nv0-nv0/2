...
function renderSmartAdvice(intelligence, orchestration){
  ...
  smartPlanAdvice.innerHTML=`<article class="card stack smart-advice-card"><div class="meta-row"><span class="pill brand">스마트 추천</span><span class="pill ${intelligence?.riskBand?.tone==='success'?'green':'gold'}">${escapeHtml(orchestration?.stage?.label||intelligence?.riskBand?.label||'추천')}</span></div><h2>${escapeHtml(next.title||intelligence?.headline||'다음 행동을 정리했습니다.')}</h2><p>${escapeHtml(next.description||intelligence?.reason||'무료 진단 결과의 확인 범위와 다음 선택지를 정리했습니다.')}</p><div class="smart-action-grid">${actions.map(item=>`<span>${escapeHtml(item)}</span>`).join('')}</div><div class="topnav"><a class="btn primary" href="${escapeHtml(path)}">${escapeHtml(next.cta||intelligence?.primaryCta||'추천 흐름 보기')}</a><a class="btn secondary" href="/portal">내 사이트 관리</a></div><small class="muted">${escapeHtml(orchestration?.caveat||intelligence?.caveat||'법률 자문이나 성과 보장을 대신하지 않습니다.')}</small></article>`;
}
