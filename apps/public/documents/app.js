...
function renderDocuments(documents = []) {
  ...
  if (!latestContent) {
    preview.innerHTML = '<div class="empty-preview">생성된 작업지시서가 여기에 표시됩니다.</div>';
    return;
  }
  preview.innerHTML = documents.map(doc => `
    <article class="document-preview stack">
      <h3>${escapeHtml(doc.title || '최종 작업지시서')}</h3>
      <div class="preview-toolbar"><span class="pill gray">작업지시서</span><span class="muted">복사 후 담당자에게 바로 전달할 수 있습니다.</span></div>
...
form?.addEventListener('submit', async (event) => {
  ...
  state.textContent = '최종 작업지시서를 생성하는 중입니다.';
  try {
    ...
    if (!data.ok) {
      state.textContent = data.error || '작업지시서 생성에 실패했습니다.';
      preview.innerHTML = '<div class="empty-preview">입력값을 확인한 뒤 다시 시도하세요.</div>';
      return;
    }
    state.textContent = '최종 작업지시서가 생성되었습니다.';
    renderDocuments(data.preview.documents || []);
  } catch (error) {
    state.textContent = `작업지시서 생성 중 오류가 발생했습니다: ${error.message}`;
    preview.innerHTML = '<div class="empty-preview">작업지시서 생성 중 오류가 발생했습니다.</div>';
  }
});

copyBtn?.addEventListener('click', async () => {
  if (!latestContent) {
    state.textContent = '복사할 작업지시서가 없습니다.';
    return;
  }
  try {
    await navigator.clipboard.writeText(latestContent);
    state.textContent = '작업지시서를 클립보드에 복사했습니다.';
  } catch {
...
loadSavedBtn?.addEventListener('click', () => {
  try {
    ...
    if (!saved.content) {
      state.textContent = '저장된 작업지시서가 없습니다.';
      return;
    }
    state.textContent = '저장된 작업지시서를 불러왔습니다.';
    renderDocuments([{ title: '저장된 최종 작업지시서', content: saved.content }]);
  } catch {
