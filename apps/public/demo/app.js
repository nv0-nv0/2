...
function renderResult(scan) {
  const findings = (scan.detailFindings || []).slice(0, 5);
  result.innerHTML = `
    <div class="result-card stack">
      ...
      <div class="kpi">${escapeHtml(scan.riskScore ?? '-')}</div>
      <p>${escapeHtml(scan.summary || '')}</p>
      <ul class="result-list">${renderList(findings, '<li>탐지 항목 없음</li>', (item) => `<li>${escapeHtml(item.title)} · ${escapeHtml(item.priority)} · 예상 최대 ${formatWon(item.estimatedPenaltyMax)}원</li>`)}</ul>
...
async function runScan() {
  ...
  try {
    const res = await fetch('/api/public/scan', {
      ...
      body: JSON.stringify({ target, turnstileToken: guard.getToken() })
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'scan failed');
    saveScan(data.result || {});
    setNextLinks(data.result || {});
    state.textContent = `스캔 완료 · ${data.result?.industry || '-'} · ${data.result?.totalFindings || 0}개 항목`;
