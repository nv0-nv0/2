...
function renderPortalDiagnosisReport(scan) {
  ...
  return `<div class="card stack portal-report-example portal-report-clean portal-unified-report"><div class="meta-row"><strong>URL 신뢰도 진단 결과</strong><span class="pill brand">실제 검사 결과</span></div><div class="portal-unified-top"><article class="portal-unified-score"><span>신뢰도 점수</span><strong>${escapeHtml(scan.riskScore ?? '-')}<em>/100</em></strong><small>${escapeHtml(riskCopy)}</small></article><article class="portal-unified-kpi"><span>개선 목표 점수</span><b>${escapeHtml(projected ?? '확인 필요')}</b><small>우선순위 항목 반영 기준</small></article><article class="portal-unified-kpi warn"><span>즉시 확인 필요</span><b>${escapeHtml(findings.length)}</b><small>상세 리포트에서 바로 수정 가능</small></article></div><div class="portal-unified-grid"><section class="portal-unified-box danger"><h4>핵심 문제</h4>${renderList(findings, '<p class="muted">세부 발견 항목 없음</p>', item => `<article><b>${escapeHtml(item.title || item.code || '점검 항목')}</b><small>${escapeHtml(item.recommendation || item.fixTemplate || '수정 방향 확인 필요')}</small></article>`)}</section><section class="portal-unified-box"><h4>항목별 분석</h4>${checks.map(item => `<article><b>${escapeHtml(item.label)}</b><small>${escapeHtml(item.score)}점 · ${escapeHtml(reportBars(item.score))}</small></article>`).join('')}</section></div><div class="portal-report-cta-row"><a class="btn primary" href="/checkout?plan=Pro&siteId=${escapeAttr(scan.siteId || '')}">상세 리포트 결제</a><a class="btn secondary" href="/plans?siteId=${escapeAttr(scan.siteId || '')}">플랜 비교</a><a class="btn secondary" href="/products/veridion/demo?target=${escapeAttr(encodeURIComponent(scan.target || ''))}">다시 진단</a></div><p class="muted">점수와 개선 목표는 내부 진단 모델 기준이며 실제 법적 안전성이나 성과를 보장하지 않습니다.</p></div>`;
}
