...
function renderPortalDiagnosisReport(scan) {
  ...
  return `<div class="card stack portal-report-example portal-report-clean"><div class="meta-row"><strong>VERIDION 진단 리포트 요약</strong><span class="pill brand">실제 검사 결과</span></div>
    ...
    <div class="portal-report-grid-clean">
      <section class="portal-report-box"><h4>항목별 분석</h4><div class="portal-report-bars clean">${checks.map(item => `<p><b>${escapeHtml(item.label)}</b><span>${escapeHtml(reportBars(item.score))}</span><small>${escapeHtml(item.score)}점</small></p>`).join('')}</div></section>
      <section class="portal-report-box"><h4>주요 발견 문제</h4><div class="portal-report-issues clean">${renderList(findings, '<p class="muted">세부 발견 항목 없음</p>', item => `<p><b>${escapeHtml(item.title || item.code || '점검 항목')}</b><span>${escapeHtml(item.recommendation || item.fixTemplate || '수정 방향 확인 필요')}</span></p>`)}</div></section>
    </div>
    <div class="portal-report-cta-row"><a class="btn primary" href="/checkout?plan=Pro&siteId=${escapeAttr(scan.siteId || '')}">상세 리포트 결제</a><a class="btn secondary" href="/plans?siteId=${escapeAttr(scan.siteId || '')}">플랜 비교</a><a class="btn secondary" href="/products/veridion/demo?target=${escapeAttr(encodeURIComponent(scan.target || ''))}">다시 진단</a></div>
    <p class="muted">점수와 개선 목표는 내부 진단 모델 기준이며 실제 법적 안전성이나 매출 개선을 보장하지 않습니다. 법률·정책 판단은 공식 원문 확인이 필요합니다.</p></div>`;
}
