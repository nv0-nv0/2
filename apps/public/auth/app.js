...
(()=>{ const url=new URL(location.href); const token=url.searchParams.get('resetToken') || ''; const email=url.searchParams.get('email') || ''; if(token && resetToken) resetToken.value=token; if(email){ if(resetConfirmEmail) resetConfirmEmail.value=email; if(resetEmail) resetEmail.value=email; if(loginEmail) loginEmail.value=email; if(registerEmail) registerEmail.value=email; } })();
(async()=>{ try{ const res = await fetch('/api/public/auth/session'); const data=await res.json(); if(data.authenticated && loginState) loginState.textContent=`${data.customer.email} 계정으로 로그인되어 있습니다.`; }catch{} })();
