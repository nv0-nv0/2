...
const PROGRESS_STEPS = [
  ...
  { percent: 82, label: '결과 저장 준비', text: '고객 포털 화면에서 이어 볼 수 있도록 결과를 저장합니다.' },
  { percent: 96, label: '자동 이동 준비', text: '결과창 표시 후 고객 포털 화면으로 연결합니다.' }
];
...
function setHomeState(message, mode = 'muted') {
  ...
  if (!state) return;
  state.className = `vr-home-demo-state ${mode}`.trim();
...
function showInputHint(input, message) {
  ...
  if (!hint) {
    hint = input.nextElementSibling?.classList?.contains('vr-input-hint') ? input.nextElementSibling : null;
  }
  if (!hint) {
...
function renderProgress(target, currentIndex = 0) {
  ...
  return `<div class="vr-home-demo-card scanning">
    <div class="vr-home-demo-card-head"><span>즉시 데모 실행 중</span><strong>${escapeHtml(domain)}</strong></div>
    <div class="vr-home-demo-progress"><i style="width:${escapeAttr(step.percent)}%"></i></div>
    <ol class="vr-home-demo-steps">${steps}</ol>
...
function renderCompleted(scan = {}, portalUrl = '/portal', endpoint = '') {
  ...
  const findingList = topFindings.length ? `<ul class="vr-home-demo-findings">${topFindings.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>` : '';
  return `<div class="vr-home-demo-card success">
    <div class="vr-home-demo-card-head"><span>무료 진단 완료</span><strong>${escapeHtml(domain)}</strong></div>
    <div class="vr-home-demo-progress complete"><i style="width:100%"></i></div>
    <div class="vr-home-demo-kpis"><article><b>${escapeHtml(score)}</b><small>개선 우선도</small></article><article><b>${escapeHtml(findings)}</b><small>발견/검토 항목</small></article><article><b>자동</b><small>고객 포털 이동</small></article></div>
    <p>결과가 저장되었습니다. <b><span id="homeDemoRedirectCountdown">2</span>초 후</b> 고객 포털 화면에서 같은 결과를 이어서 확인합니다.</p>
    ${findingList}
    <div class="vr-home-demo-actions"><a class="vr-button primary" href="${escapeAttr(safePortal)}">고객 포털에서 보기</a><a class="vr-button secondary" href="${escapeAttr(demoPageUrl(scan.target || domain))}">전용 데모로 자세히 보기</a></div>
    ${endpoint ? `<small class="vr-home-demo-endpoint">실행 API: ${escapeHtml(endpoint)}</small>` : ''}
  </div>`;
}

function renderWarning(message, normalized = '') {
  return `<div class="vr-home-demo-card warn"><div class="vr-home-demo-card-head"><span>계속 진행 필요</span><strong>${escapeHtml(compactDomain(normalized || '진단 주소'))}</strong></div><p>${escapeHtml(message)}</p><div class="vr-home-demo-actions"><a class="vr-button primary" href="${escapeAttr(demoPageUrl(normalized))}">전용 데모 페이지에서 계속</a><a class="vr-button secondary" href="/service">서비스 범위 보기</a></div></div>`;
}

function overlayHtml(target, currentIndex = 0, done = false) {
  const domain = compactDomain(target);
  const step = done ? { percent: 100, text: '진단 완료 · 결과 저장 후 고객 포털로 이동합니다.' } : PROGRESS_STEPS[Math.min(currentIndex, PROGRESS_STEPS.length - 1)];
  return `<div class="vr-home-demo-overlay-panel">
    <div class="vr-home-demo-spinner" aria-hidden="true"><span></span></div>
    <p class="vr-home-demo-overlay-kicker">서버 기반 안전 진단</p>
    <h2>${done ? '결과 저장 완료' : '진단 실행 중'}</h2>
    <p class="vr-home-demo-overlay-target">${escapeHtml(domain)}</p>
    <div class="vr-home-demo-overlay-bar"><i style="width:${escapeAttr(step.percent)}%"></i></div>
    <p class="vr-home-demo-overlay-text">${escapeHtml(step.text)}</p>
  </div>`;
}
...
async function runHomeInstantDemo(event, input, button) {
  ...
  try {
    ...
    setHomeState('진단 완료 · 결과창을 고객 포털 화면으로 자동 연결합니다.', 'success');
