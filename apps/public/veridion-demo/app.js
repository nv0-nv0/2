...
function renderPaywall(scan){
  ...
  return `<div class="result-locked"><div class="locked-content"><ul class="result-list">${renderList(hidden, '<li>상세 근거 항목</li>', item => `<li>${escapeHtml(item.title || item)}</li>`)}</ul><p>페이지별 근거, 법령 기준, 조치 문안, 수정 후보가 이어집니다.</p></div><div class="lock-box"><div class="lock-card"><div class="pill">PRO에서 확인</div><h3>전체 진단 결과는 유료 상품에서 확인하세요.</h3><p class="muted">무료는 상위 2개 요약만 제공합니다. 전체 근거와 실행 문안은 Pro 이상 상품에서 제공됩니다.</p><div class="topnav"><a class="primary" href="/plans?riskScore=${encodeURIComponent(scan.riskScore || '')}&siteId=${encodeURIComponent(scan.siteId || '')}">상품 비교</a><a class="secondary" href="/portal?siteId=${encodeURIComponent(scan.siteId || '')}">내 사이트에 저장/관리</a><a class="secondary" href="/checkout?plan=${encodeURIComponent(scan.recommendedPlan || 'Pro')}&siteId=${encodeURIComponent(scan.siteId || '')}">PRO 신청</a></div></div></div></div>`;
}
function renderResult(scan) {
  const topFindings = (scan.topFindings || []).slice(0, 2);
  result.innerHTML = `<div class="result-card stack compact-result"><div class="meta-row"><strong>${escapeHtml(scan.target || '')}</strong><span class="pill gold">${escapeHtml(scan.riskLevel || '-')}</span></div><div class="grid cols-2"><div><div class="muted">위험도</div><div class="kpi">${escapeHtml(scan.riskScore ?? '-')}</div></div><div><div class="muted">예상 최대 과태료</div><div class="kpi">${formatWon(scan.estimatedMaxPenalty)}</div></div></div><div class="notice">무료 진단은 핵심 요약만 제공합니다. 전체 근거, 페이지별 조치안, 정책 문안, 수정 후보는 유료 상품에서 제공됩니다.</div><strong>상위 위험 2개</strong><ul class="result-list">${renderList(topFindings, '<li>상위 위험 항목 없음</li>', item => `<li>${escapeHtml(item)}</li>`)}</ul><div class="upgrade-box"><strong>추천 상품: ${escapeHtml(scan.recommendedPlan || 'Pro')}</strong><p class="muted">상세 리포트와 실행 문안이 필요하면 알맞은 상품으로 이어서 진행하세요.</p><div class="topnav"><a class="btn primary" href="/plans?riskScore=${encodeURIComponent(scan.riskScore || '')}&siteId=${encodeURIComponent(scan.siteId || '')}">무료·유료 차이 보기</a><a class="btn secondary" href="/portal?siteId=${encodeURIComponent(scan.siteId || '')}">내 사이트 관리</a></div></div></div>${renderPaywall(scan)}`;
}
