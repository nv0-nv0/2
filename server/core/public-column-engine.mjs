...
const CTA_COLUMN_SEEDS = [
  ...
  },
  {
    ...
    faq: [
      ...
      ['상세한 법률 판단도 자동으로 해주나요?', 'VERIDION은 법률 위반을 확정하지 않고 공개 화면에서 보이는 리스크 후보와 보완 우선순위를 정리합니다.']
    ],
  ...
  },
  {
    ...
    faq: [
      ['nv0가 법률 위반 여부를 판단하나요?', '아니요. VERIDION은 법률 위반이나 과태료를 확정하지 않고 공개 페이지에서 보이는 리스크 후보를 정리합니다.'],
...
function buildBody(seed) {
  ...
  const cta = ['무료 진단으로 먼저 확인할 수 있는 것', `VERIDION 무료 진단은 ${seed.keyword}과 관련된 문제 영역, 영향 요소, 구분별 개수를 빠르게 보여줍니다. 무료 화면에서는 상세 근거와 수정 문구를 과하게 열어두지 않고, 어떤 영역에 리스크 후보가 몇 개 있는지 먼저 확인하게 합니다. 이후 기본 리포트와 전문가 리포트에서 페이지별 근거, 수정 문장, 적용 순서, 수동 확인 항목을 확인할 수 있습니다.`];
  const support = ['체크리스트와 FAQ', `${seed.checklist.map((item, idx) => `${idx + 1}. ${item}`).join('\n')}\n\n자주 묻는 질문\nQ. ${seed.faq[0][0]}\nA. ${seed.faq[0][1]}\nQ. ${seed.faq[1][0]}\nA. ${seed.faq[1][1]}\n\n주의: VERIDION은 법률 위반 여부나 과태료를 확정하지 않습니다. 공식 법령과 기관 기준을 바탕으로 공개 화면의 리스크 후보를 줄이는 보조 도구입니다.`];
