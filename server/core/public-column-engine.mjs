...
const SEO_TAG_POOL = {
  common: ['온라인사업자','법률리스크','규제점검','과태료리스크','전자상거래법','개인정보보호','표시광고법','무료진단','사업자점검','리스크관리'],
  ...
  conversion: ['CTA','전환율개선','버튼문구','구매전환','문의전환','랜딩페이지','모바일전환','고객신뢰','내부링크','페이지구조'],
...
const CTA_COLUMN_SEEDS = [
  {
    ...
    hook: '결제 전 고지',
    title: '결제 버튼 앞에서 빠진 안내가 과태료 리스크 후보가 되는 이유',
    ...
    tags: [...SEO_TAG_POOL.common, ...SEO_TAG_POOL.ecommerce].slice(0, 10),
    faq: [
  ...
  },
  {
    ...
    tags: [...SEO_TAG_POOL.privacy].slice(0, 10),
    faq: [
      ['문의폼에 개인정보처리방침 링크만 있으면 충분한가요?', '링크만 있는 것보다 입력 항목, 수집 목적, 답변 기준을 입력 영역 가까이에 함께 보여주는 구성이 더 이해하기 쉽습니다.'],
      ['상세한 법률 판단도 자동으로 해주나요?', 'nv0는 법률 위반을 확정하지 않고 공개 화면에서 보이는 리스크 후보와 보완 우선순위를 정리합니다.']
    ],
  ...
  },
  {
    id: 'ad-claim',
    hook: '표시광고 표현',
    title: '100% 보장 문구가 고객 신뢰를 깨는 방식',
    keyword: '표시광고 표현 점검',
    intent: '성과 보장·최고·완벽 같은 표현이 있는 광고 문구의 리스크 후보 점검',
    problem: '성과를 단정하거나 과장으로 읽힐 수 있는 문구는 전환을 높이는 대신 표시광고 리스크 후보를 만들 수 있습니다.',
    tags: [...SEO_TAG_POOL.ads].slice(0, 10),
    faq: [
      ['강한 광고 문구를 모두 없애야 하나요?', '모두 없앨 필요는 없지만 근거, 조건, 범위를 함께 제시해 오해 가능성을 줄이는 방향이 좋습니다.'],
      ['어떤 표현을 먼저 봐야 하나요?', '100% 보장, 무조건, 확정, 최고, 최저가처럼 근거 확인이 필요한 단정형 표현부터 점검하는 것이 좋습니다.']
    ],
  ...
  },
  {
    ...
    faq: [
      ...
      ['월 구독 상품도 무료 진단 대상인가요?', '공개 페이지에 있는 구독 안내, 결제 전 문구, 해지 안내 링크를 기준으로 리스크 후보를 점검할 수 있습니다.']
    ],
  ...
  },
  {
    ...
    keyword: '모바일 리스크 점검',
    intent: '모바일 화면에서 법률·규제 안내가 실제 고객에게 보이는지 확인',
    problem: 'PC에서는 보이던 환불 기준과 개인정보 링크가 모바일에서는 접혀 숨겨질 수 있습니다. 실제 고객 화면 기준으로 다시 점검해야 합니다.',
    tags: ['모바일점검','모바일가독성','정책링크','개인정보안내','환불정책','결제전고지','CTA배치','전환율개선','온라인사업자','무료진단'],
    faq: [
  ...
  },
  {
    ...
    problem: '자동 진단은 공개 화면의 리스크 후보를 줄이는 보조 도구입니다. 법률 위반 여부나 과태료를 확정하지 않는다는 고지가 필요합니다.',
    tags: ['법률자문아님','리스크후보','자동진단','과태료리스크','공개화면점검','법률리스크','규제점검','사업자점검','유료리포트','무료진단'],
    faq: [
      ['nv0가 법률 위반 여부를 판단하나요?', '아니요. nv0는 법률 위반이나 과태료를 확정하지 않고 공개 페이지에서 보이는 리스크 후보를 정리합니다.'],
  ...
  },
  {
    ...
    tags: ['공개페이지점검','고객화면','관리자화면','정책노출','사업자정보','개인정보처리방침','환불정책','검색노출','리스크관리','무료진단'],
    faq: [
      ['관리자 페이지에 입력한 정책도 검색 로봇이 읽나요?', '일반적으로 검색 로봇과 고객은 공개 URL 기준으로 접근합니다. 공개 페이지에서 보이지 않는 정보는 점검 대상에서 빠질 수 있습니다.'],
      ['공개 URL만 입력하면 되나요?', '무료 진단은 공개 URL 기준으로 확인 가능한 구조를 먼저 점검합니다. 상세 점검은 유료 리포트에서 더 넓게 정리합니다.']
    ],
    checklist: ['정책 정보가 공개 URL에서 접근 가능한가', '메뉴와 푸터에서 정책 링크가 연결되는가', '검색 로봇이 읽을 수 있는 텍스트 형태인가', '중요 안내가 이미지 안에만 들어가 있지 않은가']
  }
];
...
function buildBody(seed) {
  const interest = [
    ['왜 이 문제가 지금 중요할까요', `${seed.problem} 많은 온라인 사업자는 광고 소재, 상품 설명, 가격 혜택을 먼저 다듬지만 고객이 결제·문의 직전에 확인하는 법률·규제 안내는 뒤로 미룹니다. 문제는 이런 안내가 부족하면 고객의 불안과 문의가 늘고, 운영자는 나중에 환불·민원·분쟁 대응에 더 많은 시간을 쓰게 된다는 점입니다.`],
    ['고객과 검색 로봇이 함께 보는 구조', `검색 로봇은 제목, 요약, 소제목, 내부 링크, 반복되는 핵심 용어를 통해 문서의 주제를 이해합니다. 동시에 사람은 첫 화면에서 “이 사이트가 믿을 만한가”, “문제가 생기면 어디로 연락하나”, “결제 후 취소 기준은 무엇인가”를 확인합니다. 그래서 ${seed.hook} 관련 안내는 한 문장으로 숨기기보다 별도 소제목과 체크리스트로 분리하는 편이 좋습니다.`],
    ['사업자가 자주 놓치는 실제 상황', `관리자 화면에는 정보가 입력되어 있어도 고객 공개 페이지에는 보이지 않는 경우가 있습니다. PC에서는 보이는 정책 링크가 모바일에서는 접혀 있거나, 이미지 안에만 들어간 안내문 때문에 검색 로봇이 문맥을 충분히 읽지 못하는 경우도 있습니다. ${seed.keyword}은 이런 작은 공백을 먼저 발견하는 데서 시작합니다.`]
  ];
  const cta = ['무료 진단으로 먼저 확인할 수 있는 것', `nv0 무료 진단은 ${seed.keyword}과 관련된 문제 영역, 영향 요소, 구분별 개수를 빠르게 보여줍니다. 무료 화면에서는 상세 근거와 수정 문구를 과하게 열어두지 않고, 어떤 영역에 리스크 후보가 몇 개 있는지 먼저 확인하게 합니다. 이후 기본 리포트와 전문가 리포트에서 페이지별 근거, 수정 문장, 적용 순서, 수동 확인 항목을 확인할 수 있습니다.`];
  const support = ['체크리스트와 FAQ', `${seed.checklist.map((item, idx) => `${idx + 1}. ${item}`).join('\n')}\n\n자주 묻는 질문\nQ. ${seed.faq[0][0]}\nA. ${seed.faq[0][1]}\nQ. ${seed.faq[1][0]}\nA. ${seed.faq[1][1]}\n\n주의: nv0는 법률 위반 여부나 과태료를 확정하지 않습니다. 공식 법령과 기관 기준을 바탕으로 공개 화면의 리스크 후보를 줄이는 보조 도구입니다.`];
...
export function buildPublicColumnEnginePosts({ now = Date.now(), pageSize = 20 } = {}) {
  return CTA_COLUMN_SEEDS.map((seed, index) => {
    ...
    const metaDescription = `${seed.keyword}: ${seed.intent}. 온라인 사업자가 공개 화면에서 놓치기 쉬운 리스크 후보를 점검하고 무료 진단으로 연결합니다.`;
    const post = {
      id: seed.id,
      slug,
      title,
      ...
      metaTitle: `${title} | nv0 게시판`,
      metaDescription,
      canonicalPath: `/board#${slug}`,
      headings: ['왜 이 문제가 지금 중요할까요', '고객과 검색 로봇이 함께 보는 구조', '사업자가 자주 놓치는 실제 상황', '무료 진단으로 먼저 확인할 수 있는 것', '체크리스트와 FAQ'],
...
export function publicColumnStats(posts = []) {
  ...
  return {
    ...
    productDefinition: '온라인 사업자의 법률·규제·과태료 리스크 감소',
    seoEnhancement: {
      uniqueTitle: true,
      metaDescription: true,
      headings: true,
