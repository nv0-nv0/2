export const COMMERCIAL_READINESS_VERSION = 'commercial-readiness-v1.0.0';
...
export function runCommercialReadinessAudit({ files = [], packageJson = {}, routes = [], envExample = '' } = {}) {
  ...
  const checks = [
    { key: 'commercialReadinessModule', pass: normalized.includes('server/core/commercial-readiness.mjs'), weight: 12, message: '상용 법무/결제/운영 게이트 모듈' },
    ...
    { key: 'paymentGate', pass: envExample.includes('NV0_PAYMENT_PROVIDER=disabled') && envExample.includes('NV0_PORTONE_WEBHOOK_VERIFY_MODE=strict') && envExample.includes('NV0_ALLOW_PRELAUNCH_ONLINE_PAYMENT=false'), weight: 12, message: 'prelaunch 결제 비활성·웹훅 strict 게이트' },
    { key: 'legalGate', pass: envExample.includes('NV0_PRIVACY_POLICY_VERSION') && envExample.includes('NV0_TERMS_VERSION') && envExample.includes('NV0_REFUND_POLICY_VERSION'), weight: 12, message: '공개 정책 버전 게이트' },
    { key: 'opsGate', pass: envExample.includes('NV0_BACKUP_REMOTE_REQUIRE_ENCRYPTION=true') && envExample.includes('NV0_READYZ_REDIS_STRICT=true'), weight: 12, message: '백업 암호화·Redis strict readiness 게이트' },
    ...
    { key: 'readinessScripts', pass: Boolean(scripts['validate:commercial']) && Boolean(scripts['deploy:precheck']), weight: 8, message: '상용 검증·배포 사전점검 연결' },
    { key: 'previousGates', pass: scripts['verify:release'] === 'node scripts/run-release-gate.mjs' && scripts['release:predeploy'] === 'npm run verify:release', weight: 6, message: '단일 릴리즈 게이트 유지' }
  ];
  const score = checks.reduce((sum, item) => sum + (item.pass ? item.weight : 0), 0);
