...
const TOPIC_PACKS = [
  ...
  { ctaType: 'seo_internal_link', boardType: 'notice', primaryKeyword: '사이트 내부링크 SEO', headline: '내부링크와 CTA 정리', angle: '검색 유입 후 다음 행동으로 이동시키는 구조', intent: 'SEO 개선형', funnel: '인지', persona: 'SEO 담당자', secondaryKeywords: ['내부링크', '콘텐츠 허브'], intro: ['검색 유입은 글 하나에서 끝나지 않고 다음 페이지로 자연스럽게 이어져야 효과가 커집니다.'], problem: ['관련 글, 정책 페이지, 무료 진단 CTA가 분리되어 있으면 사용자가 다음 행동을 찾기 어렵습니다.'], cta: '내부링크와 CTA 구조는 Auto 발행 글을 통해 꾸준히 보강할 수 있습니다.' },
  { ctaType: 'seasonal_campaign', boardType: 'case', primaryKeyword: '이벤트 랜딩 점검', headline: '캠페인 전 점검', angle: '프로모션 시작 전 반드시 확인할 안내 구조', intent: '실행 준비형', funnel: '결정', persona: '프로모션 담당자', secondaryKeywords: ['이벤트 페이지', '프로모션 CTA'], intro: ['이벤트 기간에는 유입이 늘어나는 만큼 작은 안내 공백도 문의로 이어지기 쉽습니다.'], problem: ['할인 조건, 적용 제외, 환불 기준, 종료일이 분산되어 있으면 고객이 혜택보다 조건을 더 오래 찾게 됩니다.'], cta: '캠페인 전에는 무료 진단으로 핵심 안내가 한 화면 안에 연결되는지 확인하세요.' }
];
...
const INTRO_BRIDGES = [
  ...
  '검색 유입, 광고 유입, 재방문 고객 모두 같은 화면을 보더라도 필요한 정보는 조금씩 다르게 확인합니다.',
...
function legacyBuildCtaBoardArticle(scan = {}, variant = {}, options = {}) {
  ...
  return {
    ...
    seo: {
      ...
      internalLinks,
      readingTimeMinutes,
      contentGoal: `${topic.intent} 방문자를 ${topic.funnel} 단계 CTA로 연결`
    }
  };
}
...
const COMBO_FAQS = [
  ['왜 같은 주제로 글을 계속 발행해도 되나요?', '같은 큰 주제라도 업종, 고객 단계, 검색 의도, CTA 위치, FAQ가 달라지면 전혀 다른 목적의 글이 됩니다.'],
  ['무한대 결과물이 실제로 가능한가요?', '저장공간과 발행 횟수에는 한계가 있지만, 시드·시간·사이트·업종·발견항목·문체 조합을 계속 섞어 사실상 반복 없는 결과물을 만들 수 있습니다.'],
  ['SEO에서 가장 피해야 할 점은 무엇인가요?', '제목만 바꾸고 본문 구조가 같은 글을 반복 발행하는 것입니다. 그래서 본문 구조, FAQ, 내부링크, CTA까지 함께 바꿔야 합니다.'],
...
const COMBO_MICRO_CASES = [
  ...
  '광고 랜딩에는 혜택이 있지만 예외 조건이 분리되어 있는 상황', '모바일에서 CTA 버튼은 보이지만 환불·취소 기준이 접혀 있는 상황',
  'FAQ는 많지만 결제 전 불안을 직접 해결하지 못하는 상황', '검색 유입 글에서 다음 행동 링크가 없어 이탈하는 상황',
...
const HUMAN_TONE_VERSION = 'p207-4000-char-problem-aware-cta-v1';
const HARD_WORDS = [
  ['CTA', '안내 버튼'],
  ['SEO', '검색 노출'],
...
function humanizeBody({ ctx, legacy, title, titleCandidates, findingCount, top, keyword, faqSet, internalLinks, sequenceLabel }) {
  ...
  const body = [
    ...
    `문제 인식과 위기감\n${theme.weakPoint}라면 고객은 ${theme.customerQuestion}라는 질문을 해결하지 못한 채 페이지를 떠날 수 있습니다. ${micro}. ${example} 문제는 이탈이 조용히 일어난다는 점입니다. 고객은 불편하다고 말하지 않고, 문의를 남기지 않고, 그대로 다른 사이트로 이동합니다. 운영자는 광고 소재, 가격, 디자인만 문제라고 생각하기 쉽지만 실제로는 버튼 주변의 한 줄 안내가 부족해서 멈추는 경우도 많습니다. 이런 공백이 반복되면 광고비와 운영 시간은 계속 들어가는데 전환은 쌓이지 않습니다.`,
    `제품과 연관된 일반 주제\n${industry} 사이트를 보는 사람은 상품 기능만 비교하지 않습니다. 누가 운영하는지, 문제가 생기면 어디로 연락하는지, 결제 후 무엇을 받는지, 개인정보는 어떻게 쓰이는지까지 함께 확인합니다. 그래서 ${theme.label}은 단순한 정책 문구가 아니라 구매 판단을 돕는 콘텐츠입니다. 독자가 관심 있어 할 내용은 “이 제품이 좋은가”에서 끝나지 않습니다. “내가 신청해도 안전한가”, “취소나 문의가 가능한가”, “이 설명이 과장되지 않았는가”까지 이어집니다.`,
    ...
    `마지막 섹션: 자연스러운 안내\n이 글만으로 법률 판단이나 매출 상승을 단정할 수는 없습니다. 먼저 무료 진단으로 현재 상태를 확인하고, 문제가 큰 항목부터 문구와 위치를 하나씩 바꾸세요. 중요한 것은 지금 당장 모든 페이지를 다시 만드는 것이 아니라 고객이 멈추는 위치를 정확히 찾는 것입니다. 무료 진단으로 빠진 안내와 모호한 표현을 확인한 뒤, 필요할 때 상세 리포트와 수정 문구안을 이어서 확인하면 됩니다. 수정 후에는 같은 주소로 다시 점검해 남은 항목을 비교하는 것이 좋습니다.`,
    `관련 링크\n${linksText}`
  ].join('\n\n');
