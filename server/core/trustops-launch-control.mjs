...
import { buildTrustOpsAutopilotCockpit, buildAutomationWorkQueue, buildAutomationBacklog, buildNextBestOffer } from './trustops-autopilot-engine.mjs';

export const TRUSTOPS_LAUNCH_CONTROL_VERSION = 'trustops-launch-control-v1';
...
export function buildLaunchExpansionBacklog() {
  const previous = buildAutomationBacklog();
  const launchItems = [];
...
export function buildRolloutPlan(readiness = {}) {
  ...
  return [
    { stage: 0, name: 'internal_only', label: '내부 운영자 검증', traffic: '0%', enter: 'launch-control release gate 통과', exit: 'P0 0개 확인', allowed: true },
...
export function buildConversionExperimentPlan(db = {}, options = {}) {
  ...
  return { ok: true, version: TRUSTOPS_LAUNCH_CONTROL_VERSION, generatedAt: nowIso(options), experiments, activeExperimentCount: experiments.length };
}
...
export function buildLifecycleMessageSequence(input = {}, db = {}, options = {}) {
  ...
  return {
    ok: true,
    version: TRUSTOPS_LAUNCH_CONTROL_VERSION,
...
export function buildTrustOpsLaunchControl(db = {}, options = {}) {
  ...
  const backlog = buildLaunchExpansionBacklog();
  ...
  return {
    ...
    launchBacklogCount: backlog.filter(item => String(item.id).startsWith('LAUNCH-')).length,
    launchGuardrails: [
...
export function runLaunchControlAudit({ files = [], packageJson = {}, sourceText = '' } = {}) {
  const requiredFiles = [
  ...
  const launch = buildTrustOpsLaunchControl({ scans: [{ riskScore: 74, target: 'https://trustops-launch.example' }], orders: [], subscriptions: [], refundRequests: [] }, { nowIso: '2026-05-27T00:00:00.000Z' });
  ...
  check('release predeploy clean baseline', packageJson.scripts?.['release:predeploy'] === 'npm run verify:release');
  ...
  check('launch-control backlog 40', launch.launchBacklogCount === 40);
  ...
  return { ok, version: TRUSTOPS_LAUNCH_CONTROL_VERSION, score: Math.round((checks.filter(item => item.ok).length / checks.length) * 100), checks };
}
