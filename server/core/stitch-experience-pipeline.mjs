...
export function runStitchExperiencePipelinePackageAudit(input = {}) {
  ...
  const checks = [
    ...
    { key: 'all-html-local-css', pass: htmlFiles.length >= 30 && htmlFiles.every(file => /\/shared\/stitch-institutional\.css\?v=2\.7\.0/.test(htmlByFile[file])), detail: htmlFiles.filter(file => !/\/shared\/stitch-institutional\.css\?v=2\.7\.0/.test(htmlByFile[file])) },
