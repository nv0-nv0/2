...
const force = process.argv.includes('--force') || process.env.NV0_PHASE210_FORCE === 'true';
...
async function main() {
  ...
  for (const name of collections) {
    ...
    db[name] = db[name].map((item, index) => {
      ...
      const rewritten = rewriteExistingCtaPublication(item, {
        force: true,
        seed: `phase210-db-migration:${name}:${stable}:${index}`,
        sequenceOffset,
      ...
      if (force || !audit.ok || item.contentFingerprint === rewritten.contentFingerprint || String(item.body || '').length < 3800 || String(item.title || '').includes('같은')) {
        rewrittenCount += 1;
        return {
          ...item,
          ...rewritten,
          ...
          phase210Migrated: true,
          phase210MigratedAt: new Date().toISOString()
        };
      }
      rewrittenCount += 1;
      return {
  ...
  if (!dryRun) {
    const backupPath = `${dbPath}.phase210-backup-${Date.now()}`;
