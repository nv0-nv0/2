...
add('01 package version is phase272 or phase273', /phase272-premium-redesign|phase273-package-100/.test(pkg.version));
add('02 final review points phase272 or later final', ['npm run phase272:final','npm run phase273:final'].includes(pkg.scripts?.['final:review']));
...
add('10 homepage infographic flow added', home.includes('phase272-home-flow') && home.includes('주소 입력부터 내 사이트 관리까지') && css.includes('.phase272-flow-grid'));
add('11 homepage factual infographic added', home.includes('phase272-home-facts') && home.includes('읽기 전용') && !home.includes('기업 로고'));
...
add('16 plans factual prices retained', ['₩0','₩49,000','₩149,000'].every(token => plans.includes(token)));
...
add('21 board page keeps API-only insight wording', board.includes('실제 게시판 API') && board.includes('20분마다 1건 발행'));
