...
const serverFiles = [
  ...
  path.join(root, 'server', 'routes', 'public.mjs'),
  path.join(root, 'server', 'routes', 'payment.mjs')
];
...
for (const offer of offers) {
  ...
  assert.ok(server.includes(`plan === '${offer}'`) || server.includes(`offer.code`), `fulfillment path missing: ${offer}`);
}
...
assert.ok(plansHtml.includes('plans-grid') && plansHtml.includes('전문가 플랜'), 'pricing comparison grid missing');
const homeHtml = fs.readFileSync(path.join(root, 'apps/public', 'home', 'index.html'), 'utf8');
assert.ok(homeHtml.includes('/business-info') && homeHtml.includes('/board'), 'home primary menu entries missing');
assert.ok(homeHtml.includes('/plans') || homeHtml.includes('요금 안내'), 'home pricing preview missing');
