...
add('01 package version phase270', /phase270-full-package-verified-hardened/.test(pkg.version));
add('02 final review points phase270', pkg.scripts?.['final:review'] === 'npm run phase270:final');
add('03 RUN_ALL uses phase270 final gate', read('RUN_ALL_TESTS.sh').includes('npm run phase270:final'));
...
add('08 CI workflow uses phase270 final', exists('.github/workflows/ci.yml') && read('.github/workflows/ci.yml').includes('npm run phase270:final'));
add('09 e2e accepts current release line', read('tests/e2e.mjs').includes('phase270-full-package-verified-hardened'));
add('10 old validators accept phase270 release line', ['scripts/validate-phase258-structural-hardening.mjs','scripts/validate-phase259-demo-penalty-dashboard.mjs','scripts/validate-phase260-dispute-safe-penalty.mjs'].every(file => read(file).includes('phase270-full-package-verified-hardened')));
...
const report = { generatedAt: new Date().toISOString(), phase: 'phase270-full-package-verified-hardened', ok: failed === 0, total: checks.length, passed, failed, checks };
