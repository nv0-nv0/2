...
add('01 package version phase270, phase271, phase272, or phase273', /phase270-full-package-verified-hardened|phase271-site-ux-insight-polish|phase272-premium-redesign|phase273-package-100/.test(pkg.version));
add('02 final review points current final gate', ['npm run phase270:final','npm run phase271:final','npm run phase272:final','npm run phase273:final'].includes(pkg.scripts?.['final:review']));
add('03 RUN_ALL uses current final gate', read('RUN_ALL_TESTS.sh').includes('npm run phase270:final') || read('RUN_ALL_TESTS.sh').includes('npm run phase271:final') || read('RUN_ALL_TESTS.sh').includes('npm run phase272:final') || read('RUN_ALL_TESTS.sh').includes('npm run phase273:final'));
...
add('08 CI workflow uses current final', exists('.github/workflows/ci.yml') && (read('.github/workflows/ci.yml').includes('npm run phase270:final') || read('.github/workflows/ci.yml').includes('npm run phase271:final') || read('.github/workflows/ci.yml').includes('npm run phase272:final') || read('.github/workflows/ci.yml').includes('npm run phase273:final')));
add('09 e2e accepts current release line', read('tests/e2e.mjs').includes('phase273-package-100'));
add('10 old validators accept current release line', ['scripts/validate-phase258-structural-hardening.mjs','scripts/validate-phase259-demo-penalty-dashboard.mjs','scripts/validate-phase260-dispute-safe-penalty.mjs'].every(file => read(file).includes('phase273-package-100')));
