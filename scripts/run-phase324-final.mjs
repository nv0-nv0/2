...
const steps = [
  ...
  ['validate:phase324', ['npm','run','validate:phase324']],
...
for (const [name, cmd] of steps) {
  ...
  const run = spawnSync(cmd[0], cmd.slice(1), { cwd: root, encoding: 'utf8', shell: false, maxBuffer: 40 * 1024 * 1024, env: { ...process.env, NV0_PHASE324_GATE: '1' } });
  const elapsedMs = Date.now() - start;
  const ok = run.status === 0;
  const record = { name, command: cmd.join(' '), ok, status: run.status, elapsedMs, stdoutTail: String(run.stdout || '').slice(-2500), stderrTail: String(run.stderr || '').slice(-2500) };
  results.push(record);
...
const report = { ok: failed === 0 && passed === steps.length, phase: 'phase324-complete-delivery', checkedAt: new Date().toISOString(), passed, failed, total: steps.length, results };
fs.writeFileSync(path.join(root, 'docs/current/PHASE324_FINAL_GATE_LOG.txt'), results.map(item => `${item.ok ? 'PASS' : 'FAIL'} ${item.name} ${item.elapsedMs}ms\n${item.stderrTail || ''}`).join('\n'));
fs.writeFileSync(path.join(root, 'docs/current/PHASE324_FINAL_GATE_REPORT.json'), JSON.stringify(report, null, 2));
if (!report.ok) {
  console.error(JSON.stringify(report, null, 2));
  process.exit(1);
}
console.log(JSON.stringify({ ok: true, phase: report.phase, passed, total: steps.length, report: 'docs/current/PHASE324_FINAL_GATE_REPORT.json' }, null, 2));
