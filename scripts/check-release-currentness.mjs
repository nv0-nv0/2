...
add('package-version-phase347', () => assert.match(pkg.version, /phase347-unified-diagnosis-final|phase348-final-unified-engine-closeout|phase349-customer-journey-closeout|phase350-global-cta-semantics-closeout|phase351-prompt-full-sweep-closeout/));
add('description-current-final', () => assert.match(pkg.description || '', /phase347 unified diagnosis|phase348 final unified diagnosis engine|phase349 customer journey|phase350 global cta semantics|phase351 prompt full sweep/i));
add('delivery-final-current', () => assert.ok(['npm run phase347:final','npm run phase348:final','npm run phase349:final|npm run phase350:final', 'npm run phase350:final','npm run phase351:final'].includes(scripts['delivery:final'])));
add('release-predeploy-current', () => assert.ok(['npm run phase347:final','npm run phase348:final','npm run phase349:final|npm run phase350:final', 'npm run phase350:final','npm run phase351:final'].includes(scripts['release:predeploy'])));
add('phase347-final-runner', () => assert.equal(scripts['phase347:final'], 'node scripts/run-phase347-final.mjs'));
add('run-all-tests-current', () => assert.match(read('RUN_ALL_TESTS.sh'), /npm run phase347:final|npm run phase348:final|npm run phase349:final|npm run phase350:final|npm run phase351:final/));
add('readme-current-command', () => assert.match(read('README.md'), /npm run phase347:final|npm run phase348:final|npm run phase349:final|npm run phase350:final|npm run phase351:final/));
