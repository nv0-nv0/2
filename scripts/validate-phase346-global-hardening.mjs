...
add('version:phase346', /phase346-global-hardening-final/.test(pkg.version), pkg.version);
add('scripts:phase346-final', pkg.scripts?.['phase346:final'] === 'node scripts/run-phase346-final.mjs', pkg.scripts?.['phase346:final']);
add('scripts:terminal-gates', pkg.scripts?.['delivery:final'] === 'npm run phase346:final' && pkg.scripts?.['release:predeploy'] === 'npm run phase346:final', `${pkg.scripts?.['delivery:final']} | ${pkg.scripts?.['release:predeploy']}`);
...
add('readme:phase346', /phase346/.test(read('README.md')) && /live:smoke/.test(read('README.md')));
add('run-all-tests:phase346', /phase346:final/.test(read('RUN_ALL_TESTS.sh')));
