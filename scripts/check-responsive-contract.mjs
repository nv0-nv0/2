...
const legacyCss = ['phase307-professional-polish.css','phase309-portal-screenshot-repair.css','portal-phase283-dashboard.css','nv0n-runtime.css','nv0n-generated.css'];
...
for (const token of legacyCss) {
  if (text.includes(token) || css.includes(token)) failures.push(`legacy css reference must not return: ${token}`);
}
const brokenGlyph = /[�□■◆◇●▲▼※★☆♣♥♠♬✓✔✕✖↔⇒⇐⇔⌕▱↻▤▥♢⚖⚙☑⋮🛡█░›↗]/u;
...
for (const bp of requiredBreakpoints) {
  if (!css.includes(bp)) failures.push(`missing responsive breakpoint: ${bp}`);
}
const requiredClasses = ['.v-shell','.v-nav','.v-hero','.v-grid','.v-card','.v-button'];
...
const report = { ok: failures.length === 0, phase: 'phase323-responsive-contract', checkedFiles: publicFiles.length, requiredBreakpoints, failures };
fs.mkdirSync(path.join(root, 'docs/current'), { recursive: true });
fs.writeFileSync(path.join(root, 'docs/current/PHASE323_RESPONSIVE_CONTRACT.json'), JSON.stringify(report, null, 2));
if (failures.length) {
