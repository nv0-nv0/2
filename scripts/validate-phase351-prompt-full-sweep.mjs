...
add('version:phase351-or-newer', () => assert.match(pkg.version, /1\.0\.13-commercial-phase351-prompt-full-sweep-closeout|1\.0\.14-commercial-phase353-full-package-closeout|1\.0\.15-commercial-phase354-deployment-security-closeout|1\.0\.16-commercial-phase355-organization-closeout|phase356-conversion-dashboard-closeout|phase357-global-qa-accessibility-closeout|phase358-commercial-deploy-integrity-closeout/));
...
add('scripts:delivery-release-current', () => {
  assert.ok(['npm run phase351:final','npm run phase353:final','npm run phase354:final','npm run phase355:final','npm run phase356:final','npm run phase357:final','npm run phase358:final'].includes(pkg.scripts['delivery:final']));
  assert.ok(['npm run phase351:final','npm run phase353:final','npm run phase354:final','npm run phase355:final','npm run phase356:final','npm run phase357:final','npm run phase358:final'].includes(pkg.scripts['release:predeploy']));
});
...
add('readme:terminal-gate', () => assert.match(read('README.md'), /phase353:final|phase354:final|phase355:final|phase356:final|phase357:final|npm run phase358:final/));
add('run-all-tests:terminal-gate', () => assert.match(read('RUN_ALL_TESTS.sh'), /phase353:final|phase354:final|phase355:final|phase356:final|phase357:final|npm run phase358:final/));
