...
for (const token of ['Commercial launch requires NV0_PERSISTENCE_MODE=postgres_primary','Commercial launch requires NV0_SESSION_STORE=redis','Commercial launch requires NV0_LOCK_PROVIDER=redis','putObjectToS3Compatible',"if (PLATFORM.commercial && ['/demo', '/products/veridion/demo'].includes(urlPath)) return null",'BUSINESS_PROFILE','584-77-00586']) if (!server.includes(token)) fail.push(`server missing launch guard/profile: ${token}`);
