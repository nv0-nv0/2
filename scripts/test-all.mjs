...
for (const token of ['NV0','무료 진단 시작','광고비 쓰기 전에','결제 전 신뢰','Risk 72 / 100','추천 구매 순서','Pro 리포트','법률 자문 서비스인가요?']) add(`home:${token}`, home.includes(token));
...
for (const token of ['무료 요약 진단 3회','사이트 주소 하나로','결제 전 신뢰 리스크','무료·유료 비교','Pro 리포트 신청']) add(`demo:${token}`, demo.includes(token));
...
for (const token of ['상품·요금','Pro 리포트','정기 점검','전체 상품 비교','무료 진단 시작']) add(`plans:${token}`, plans.includes(token));
...
add('documents:minimal-fields', docs.includes('전화번호·주소는 필요한 경우에만 선택 입력'));
