...
for (const token of ['NV0','무료 진단 시작','광고를 시작하기 전에','결제 전 신뢰 점검','검사 후 표시','고정 예시 점수 없음','추천 이용 순서','상세 리포트','법률 자문 서비스인가요?']) add(`home:${token}`, home.includes(token));
...
for (const token of ['상품·요금','상세 리포트','Auto 정기 케어','상품 바로 보기','무료 진단 시작']) add(`plans:${token}`, plans.includes(token));
add('plans:not-stuck-loading', !plans.includes('불러오는 중입니다'));
add('plans:static-fallback-cards', plans.includes('39,000원') && plans.includes('월 149,000원'));
