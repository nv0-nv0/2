...
const excludedPrefixes = ['.git','node_modules','docs/current','runtime/uploads','runtime/backups','runtime/reports','runtime/data/secure-records','coverage','.DS_Store'];
...
function excluded(rel) { const n = normalize(rel); return forbiddenEnv(n) || forbiddenRuntime.has(n) || excludedPrefixes.some(prefix => n === prefix || n.startsWith(`${prefix}/`)) || n.startsWith('runtime-test-'); }
...
const suspicious=files.filter(rel => forbiddenEnv(rel) || forbiddenRuntime.has(rel) || rel.startsWith('runtime-test-'));
