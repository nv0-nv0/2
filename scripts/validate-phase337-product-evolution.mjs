...
for (const file of publicFiles) {
  ...
  check(`${file} public nav`, () => assert.match(html, /무료 진단 시작/));
}
...
['itemscope itemtype="https://schema.org/Article"','실무 체크리스트','인사이트 FAQ','무료 진단 시작','리포트 요금 보기','검색 로봇'].forEach(token => check(`board has ${token}`, () => assert.ok(board.includes(token))));
