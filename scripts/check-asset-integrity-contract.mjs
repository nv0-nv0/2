...
import fs from 'node:fs'; import path from 'node:path'; import { fileURLToPath } from 'node:url'; import { ASSET_VERSION } from '../shared/release-version.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'); const version=ASSET_VERSION;
const walk=d=>fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(path.join(d,e.name)):[path.join(d,e.name)]);
const htmlFiles=walk(path.join(root,'apps')).filter(f=>f.endsWith('.html')); const errors=[]; let refs=0;
for(const file of htmlFiles){const text=fs.readFileSync(file,'utf8'); const matches=[...text.matchAll(/(?:href|src)=["'](\/(?:shared|apps)\/[^"']+\.(?:css|js)(?:\?[^"']*)?)["']/g)]; for(const [,url] of matches){refs++; if(new URL(url,'https://nv0.kr').searchParams.get('v')!==version) errors.push({file:path.relative(root,file),url,error:'local asset missing or stale release version'});}}
