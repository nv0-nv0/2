import crypto from 'node:crypto';

function randBase64(bytes = 32) {
  return crypto.randomBytes(bytes).toString('base64url');
}
function randHex(bytes = 32) {
  return crypto.randomBytes(bytes).toString('hex');
}
function randBase32(bytes = 20) {
  const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567';
  const data = crypto.randomBytes(bytes);
  let bits = '', out = '';
  for (const byte of data) bits += byte.toString(2).padStart(8, '0');
  for (let i = 0; i < bits.length; i += 5) out += alphabet[Number.parseInt(bits.slice(i, i + 5).padEnd(5, '0'), 2)];
  return out;
}
function value(name, fallback = '') {
  const raw = String(process.env[name] || '').trim();
  return raw || fallback;
}

const accountId = value('R2_ACCOUNT_ID', 'R2_ACCOUNT_ID');
const bucket = value('R2_BUCKET', 'nv0-production');
const r2AccessKey = value('R2_ACCESS_KEY_ID', 'R2_ACCESS_KEY_ID');
const r2SecretKey = value('R2_SECRET_ACCESS_KEY', 'R2_SECRET_ACCESS_KEY');
const postgresPassword = value('POSTGRES_PASSWORD', randBase64(24));
const adminPassword = value('NV0_BOOTSTRAP_ADMIN_PASSWORD', randBase64(24));
const sessionSecret = value('NV0_SESSION_SECRET', randBase64(48));
const secureKey = value('NV0_SECURE_RECORDS_KEY', randBase64(48));
const secureSalt = value('NV0_SECURE_RECORDS_SALT', randHex(32));
const privacyHashKey = value('NV0_PRIVACY_HASH_KEY', randBase64(48));
const backupEncryptionSecret = value('NV0_BACKUP_ENCRYPTION_SECRET', randBase64(48));
const adminTotpSecret = value('NV0_ADMIN_TOTP_SECRET', randBase32(20));

const lines = [
  '# VERIDION strict commercial Coolify env - R2 primary profile',
  '# Pair this output with deploy/docker-compose.commercial.yml.',
  '# Do not paste this strict profile into the boot-safe root docker-compose.yml.',
  '# Replace every REPLACE_REAL_* and R2_* placeholder before deployment.',
  '# Keep payment disabled until commercial_launch cutover and webhook verification are complete.',
  'NODE_ENV=production',
  'HOST=0.0.0.0',
  'PORT=3210',
  'NV0_RUNTIME_DIR=/tmp/nv0-runtime',
  'NV0_FALLBACK_RUNTIME_DIR=/tmp/nv0-runtime',
  'NV0_REQUIRE_PERSISTENT_RUNTIME=auto',
  'NV0_PLATFORM_TARGET=commercial',
  'NV0_DEPLOYMENT_STAGE=prelaunch',
  'NV0_COMMERCIAL_LAUNCH_READY=false',
  'NV0_RUN_PREFLIGHT=true',
  'NV0_EXPOSE_INTERNAL_PUBLIC_APIS=false',
  'NV0_PUBLIC_BASE_URL=https://nv0.kr',
  'NV0_CANONICAL_HOST_REDIRECT=false',
  'NV0_REDIRECT_OWNER=edge',
  'NV0_DEPLOYMENT_RISK_STRICT=false',
  'NV0_TRUST_PROXY_HEADERS=true',
  'NV0_ALLOWED_ADMIN_ORIGINS=nv0.kr,www.nv0.kr',
  'NV0_ALLOWED_HOSTS=nv0.kr,www.nv0.kr',
  'NV0_REQUEST_TIMEOUT_MS=15000',
  'NV0_SLOW_REQUEST_THRESHOLD_MS=1500',
  'NV0_ACCESS_LOG_MODE=normal',
  'NV0_LOG_HEALTHCHECK_REQUESTS=false',
  'NV0_LOG_FAVICON_REQUESTS=false',
  'NV0_HEALTHZ_STRICT=false',
  'NV0_READYZ_REDIS_STRICT=true',
  'NV0_READYZ_CACHE_TTL_MS=3000',
  'NV0_BUILD_VERSION=',
  'NV0_BUILD_TIME=',
  'NV0_COMMIT_SHA=',
  'NV0_RELEASE_ID=',
  'NV0_MAX_JSON_BODY_BYTES=1048576',
  'NV0_MAX_MULTIPART_BODY_BYTES=10485760',
  'NV0_SUPPORT_EMAIL=ct@nv0.kr',
  `NV0_SESSION_SECRET=${sessionSecret}`,
  'NV0_PRIVACY_OFFICER_EMAIL=ct@nv0.kr',
  'NV0_OPERATOR_ALERT_EMAIL=ct@nv0.kr',
  'NV0_HOSTING_PROVIDER=Coolify/Contabo',
  'NV0_CUSTOMER_SERVICE_PHONE=이메일전용고객지원',
  `NV0_BUSINESS_TRADE_NAME=${value('NV0_BUSINESS_TRADE_NAME', 'REPLACE_REAL_BUSINESS_TRADE_NAME')}`,
  `NV0_BUSINESS_REPRESENTATIVE=${value('NV0_BUSINESS_REPRESENTATIVE', 'REPLACE_REAL_BUSINESS_REPRESENTATIVE')}`,
  `NV0_BUSINESS_REGISTRATION_NUMBER=${value('NV0_BUSINESS_REGISTRATION_NUMBER', 'REPLACE_REAL_BUSINESS_REGISTRATION_NUMBER')}`,
  `NV0_BUSINESS_ADDRESS=${value('NV0_BUSINESS_ADDRESS', 'REPLACE_REAL_BUSINESS_ADDRESS')}`,
  `NV0_MAIL_ORDER_REGISTRATION_NUMBER=${value('NV0_MAIL_ORDER_REGISTRATION_NUMBER', '')}`,
  'NV0_DATA_RETENTION_DAYS=90',
  'NV0_DATA_DESTRUCTION_GRACE_DAYS=30',
  'NV0_REFUND_REQUEST_WINDOW_DAYS=7',
  'NV0_RULES_VERSION=production-20260427',
  'NV0_ADMIN_AUTH_MODE=account_rbac',
  'NV0_ADMIN_MFA_REQUIRED=true',
  `NV0_ADMIN_TOTP_SECRET=${adminTotpSecret}`,
  'NV0_BOOTSTRAP_ADMIN_EMAIL=admin@nv0.kr',
  'NV0_BOOTSTRAP_ADMIN_NAME=VERIDION_Admin',
  `NV0_BOOTSTRAP_ADMIN_PASSWORD=${adminPassword}`,
  'NV0_ADMIN_SESSION_TTL_MS=3600000',
  `NV0_ADMIN_IP_ALLOWLIST=${value('NV0_ADMIN_IP_ALLOWLIST', 'REPLACE_REAL_ADMIN_PUBLIC_IP')}`,
  'NV0_ENABLE_TURNSTILE=true',
  `NV0_TURNSTILE_SITE_KEY=${value('NV0_TURNSTILE_SITE_KEY', 'REPLACE_REAL_TURNSTILE_SITE_KEY')}`,
  `NV0_TURNSTILE_SECRET=${value('NV0_TURNSTILE_SECRET', 'REPLACE_REAL_TURNSTILE_SECRET')}`,
  'POSTGRES_DB=nv0',
  'POSTGRES_USER=nv0',
  `POSTGRES_PASSWORD=${postgresPassword}`,
  'NV0_PERSISTENCE_MODE=postgres_primary',
  `NV0_DATABASE_URL=postgres://nv0:${postgresPassword}@postgres:5432/nv0`,
  'PGSSLMODE=disable',
  'NV0_DB_COMPARE_MODE=0',
  'NV0_PRELAUNCH_DB_FALLBACK=false',
  'NV0_REDIS_URL=redis://redis:6379/0',
  'NV0_REDIS_TIMEOUT_MS=1500',
  'NV0_SESSION_STORE=redis',
  'NV0_RATE_LIMIT_STORE=redis',
  'NV0_LOCK_PROVIDER=redis',
  'NV0_STORAGE_MODE=s3',
  `NV0_S3_ENDPOINT=https://${accountId}.r2.cloudflarestorage.com`,
  `NV0_S3_BUCKET=${bucket}`,
  'NV0_S3_REGION=auto',
  `NV0_S3_ACCESS_KEY_ID=${r2AccessKey}`,
  `NV0_S3_SECRET_ACCESS_KEY=${r2SecretKey}`,
  'NV0_S3_PUBLIC_BASE_URL=',
  'NV0_S3_FORCE_PATH_STYLE=true',
  'NV0_SCAN_PROVIDER=external_http',
  `NV0_SCAN_PROVIDER_URL=${value('NV0_SCAN_PROVIDER_URL', 'REPLACE_REAL_SCAN_PROVIDER_URL')}`,
  `NV0_SCAN_PROVIDER_TOKEN=${value('NV0_SCAN_PROVIDER_TOKEN', '')}`,
  'NV0_SCAN_PROVIDER_FALLBACK=true',
  'NV0_SCAN_CACHE_TTL_MS=300000',
  'NV0_SCAN_SOFT_TIMEOUT_MS=6500',
  'NV0_PUBLIC_DEMO_FORCE_SCAN_FALLBACK=true',
  'NV0_TARGET_FETCH_ENABLED=false',
  'NV0_TARGET_FETCH_MAX_PAGES=12',
  'NV0_TARGET_FETCH_CONCURRENCY=4',
  'NV0_TARGET_FETCH_TIMEOUT_MS=3000',
  'NV0_TARGET_FETCH_MAX_BYTES=524288',
  'NV0_TARGET_FETCH_MAX_REDIRECTS=3',
  'NV0_TARGET_FETCH_ROBOTS_ENABLED=true',
  'NV0_TARGET_FETCH_SITEMAP_ENABLED=true',
  'NV0_TARGET_FETCH_MAX_SITEMAP_URLS=40',
  'NV0_TARGET_FETCH_MAX_DISCOVERY_RESOURCES=4',
  'NV0_TARGET_FETCH_AUTOMATION_LEVEL=maximum_free_safe',
  'NV0_PAYMENT_PROVIDER=disabled',
  'NV0_ALLOW_PRELAUNCH_ONLINE_PAYMENT=false',
  'NV0_PAYMENT_PROVIDER_URL=',
  'NV0_PAYMENT_PROVIDER_TOKEN=',
  'NV0_PAYMENT_REDIRECT_ALLOWED_HOSTS=',
  'NV0_PORTONE_API_BASE_URL=https://api.portone.io',
  'NV0_PORTONE_API_SECRET=',
  'NV0_PORTONE_STORE_ID=',
  'NV0_PORTONE_CHANNEL_KEY=',
  'NV0_PORTONE_REDIRECT_URL=https://nv0.kr/checkout',
  'NV0_PORTONE_PAY_METHOD=CARD',
  'NV0_PORTONE_CUSTOMER_ID_PREFIX=nv0',
  'NV0_PORTONE_WEBHOOK_SECRET=',
  'NV0_PORTONE_WEBHOOK_VERIFY_MODE=strict',
  `NV0_SMTP_URL=${value('NV0_SMTP_URL', 'REPLACE_REAL_SMTP_URL')}`,
  'NV0_EMAIL_FROM=ct@nv0.kr',
  'NV0_EMAIL_MAX_RETRY_COUNT=5',
  'NV0_EMAIL_RETRY_BACKOFF_MS=300000',
  'NV0_PUBLIC_SCAN_LIMIT=20',
  'NV0_PUBLIC_SCAN_WINDOW_MS=60000',
  'NV0_ADMIN_AUTH_LIMIT=8',
  'NV0_ADMIN_AUTH_WINDOW_MS=600000',
  'NV0_BACKUP_RETENTION_COUNT=20',
  'NV0_BACKUP_REMOTE_ENABLED=true',
  'NV0_BACKUP_REMOTE_PREFIX=backups/nv0',
  'NV0_BACKUP_COMPRESS=true',
  `NV0_BACKUP_ENCRYPTION_SECRET=${backupEncryptionSecret}`,
  'NV0_BACKUP_REMOTE_REQUIRE_ENCRYPTION=true',
  'NV0_AUTO_BACKUP_ENABLED=true',
  'NV0_AUTO_BACKUP_ON_STARTUP=true',
  'NV0_AUTO_BACKUP_INTERVAL_MS=21600000',
  'NV0_AUDIT_LOG_RETENTION_COUNT=1000',
  'NV0_PAYMENT_IDEMPOTENCY_TTL_MS=86400000',
  'NV0_PUBLIC_CACHE_SECONDS=60',
  'NV0_PUBLIC_ASSET_CACHE_SECONDS=31536000',
  'NV0_CTA_AUTOPUBLISH_INTERVAL_MS=1200000',
  `NV0_SECURE_RECORDS_KEY=${secureKey}`,
  `NV0_SECURE_RECORDS_SALT=${secureSalt}`,
  'NV0_SECURE_RECORDS_DIR=/tmp/nv0-runtime/data/secure-records',
  `NV0_PRIVACY_HASH_KEY=${privacyHashKey}`,
  'NV0_LEGAL_EVIDENCE_VERSION=legal-evidence-v1',
  'NV0_PRIVACY_POLICY_VERSION=',
  'NV0_TERMS_VERSION=',
  'NV0_REFUND_POLICY_VERSION=',
  'NV0_AI_REVIEW_PROVIDER=disabled',
  'NV0_GEMINI_API_KEY=',
  'NV0_GEMINI_MODEL=gemini-2.5-flash'
];

console.log(lines.join('\n'));
