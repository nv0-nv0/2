...
add('readme-current-command', () => assert.match(read('README.md'), /npm run phase(?:353|354|355):final/));
add('run-all-tests-current-command', () => assert.match(read('RUN_ALL_TESTS.sh'), /npm run phase(?:353|354|355):final/));
add('package-current-scripts', () => { const pkg = JSON.parse(read('package.json')); assert.match(pkg.version, /phase353-full-package-closeout|phase354-deployment-security-closeout|phase355-organization-closeout/); assert.ok(['npm run phase353:final','npm run phase354:final','npm run phase355:final'].includes(pkg.scripts['delivery:final'])); assert.ok(['npm run phase353:final','npm run phase354:final','npm run phase355:final'].includes(pkg.scripts['release:predeploy'])); assert.equal(pkg.scripts['phase353:final'], 'node scripts/run-phase353-final.mjs'); });
