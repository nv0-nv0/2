...
add('package version phase260', /phase260-dispute-safe-penalty/.test(pkg.version));
['test:phase260','validate:phase260','phase260:final'].forEach(script => add(`package script:${script}`, !!pkg.scripts?.[script]));
add('final review points to phase260', /phase260:final/.test(pkg.scripts?.['final:review'] || ''));
