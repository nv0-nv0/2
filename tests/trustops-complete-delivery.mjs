...
import { buildTrustOpsCompleteDelivery, runCompleteDeliveryAudit, TRUSTOPS_COMPLETE_DELIVERY_VERSION } from '../server/core/trustops-complete-delivery.mjs';
...
const scripts = {
  ...
  'test:trustops': 'node tests/trustops-final-handoff.mjs',
  'delivery:final': 'npm run verify:release',
  'release:predeploy': 'npm run verify:release',
...
assert.match(TRUSTOPS_COMPLETE_DELIVERY_VERSION, /trustops-complete-delivery/);
assert.equal(delivery.ok, true);
assert.equal(delivery.packageScore, 100);
assert.equal(delivery.failed.length, 0);
assert.equal(delivery.linkedScores.scorecardPackageScore, 100);
assert.ok(delivery.finalOperatorPack.length >= 10);
assert.equal(delivery.operationalTruth.liveServerScore, null);
const audit = runCompleteDeliveryAudit({ env, files, scripts, routes: ['/api/public/privacy-status','/api/public/live-verification-checklist','/api/admin/trustops-final-handoff','/api/admin/trustops-production-sentinel','/api/public/trustops-100-final','/api/public/trustops-complete-delivery'], sourceText, runtimeClean: true, secretHygienePassed: true });
...
console.log('trustops complete delivery integration ok');
