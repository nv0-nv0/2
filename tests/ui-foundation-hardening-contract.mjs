...
for(const file of html){const source=fs.readFileSync(file,'utf8');const fileRel=rel(file);add(`${fileRel}:loads-foundation-last`,()=>{assert.match(source,/\/shared\/nv0-ui-foundation\.css\?v=2\.7\.1/);const lastCss=[...source.matchAll(/<link[^>]+href="([^"]+\.css\?v=2\.7\.1)"/g)].at(-1)?.[1];assert.equal(lastCss,'/shared/nv0-ui-foundation.css?v=2.7.1');});add(`${fileRel}:loads-ui-runtime`,()=>assert.match(source,/\/shared\/nv0-ui-runtime\.js\?v=2\.7\.1/));}
