...
check('scripts/acceptance.mjs:production-shape-env-isolated-and-current-report', () => {
  const text = read('scripts/acceptance.mjs');
  assert.match(text, /env\.production\.nv0\.kr\.ci-check\.env/);
  assert.match(text, /NV0_ADMIN_KEY: ''/);
  assert.match(text, /PHASE357_ACCEPTANCE_REPORT\.json/);
});
check('scripts/acceptance.mjs:always-cleans-release-runtime', () => {
  const text = read('scripts/acceptance.mjs');
  assert.match(text, /runtime-test-acceptance/);
  assert.match(text, /scripts\/clean-release-runtime\.mjs/);
});
...
console.log(JSON.stringify({ ok: failures.length === 0, contract: 'phase357-test-runtime-isolation-contract', checked: checks.length, failed: failures.length, failures, checks }, null, 2));
