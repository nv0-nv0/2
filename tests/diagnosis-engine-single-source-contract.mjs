...
for (const [name, html] of Object.entries({ demo })) {
  add(`${name}:single-source-body-marker`, () => assert.match(html, /data-diagnosis-engine="single-source"/));
  add(`${name}:loads-canonical-demo-js`, () => assert.match(html, /<script src="\/apps\/public\/demo\/app\.js(?:\?v=[^"]+)?" type="module"><\/script>/));
  add(`${name}:loads-bootstrap-guard`, () => assert.match(html, /<script src="\/apps\/public\/demo\/bootstrap\.js(?:\?v=[^"]+)?"><\/script>/));
  ...
  add(`${name}:primary-action-copy-consistent`, () => assert.match(html, /무료 진단 시작/));
}
add('home:routes-to-canonical-demo-without-embedding-engine', () => {
  ...
  assert.match(home, /<script src="\/apps\/public\/home\/app\.js(?:\?v=[^"]+)?" type="module"><\/script>/);
  assert.doesNotMatch(home, /\/apps\/public\/demo\/app\.js/);
});
add('duplicate-demo-directory:removed', () => assert.equal(fs.existsSync(path.join(root, 'apps/public/veridion-demo')), false));
