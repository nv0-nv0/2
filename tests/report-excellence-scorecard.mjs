...
add('문서 통제와 기관 보고서 형식', 10, hasAll(js, ['REPORT ID','REPORT CLASS','PUBLIC WEB SIGNALS','ISSUED','CLIENT CONFIDENTIAL','FREE PREVIEW · CONTROLLED DISCLOSURE']), '보고서 ID·등급·범위·발행일·통제 공개 표기');
add('경영진 의사결정 요약', 10, hasAll(js, ['MANAGEMENT DECISION','01 · EXECUTIVE DECISION','WHY IT MATTERS','WHAT IS VERIFIED','WHAT TO DO NEXT']), '경영진이 즉시 읽는 판단 요약 3단 구조');
add('신뢰 노출 지수와 산정 한계', 8, hasAll(js, ['TRUST EXPOSURE INDEX','공개 신호 상대 비교','법률 판단이나 실제 매출 손실 확정값이 아니라','공개 화면에서 우선 점검해야 할 신뢰 공백의 상대 강도']), '위험 지수·상대 비교·과장 방지 문구');
add('무료 25%와 유료 75% 경계', 10, hasAll(js + html, ['FREE EXECUTIVE BRIEF · 25% OPEN','CONTROLLED DISCLOSURE · 75% LOCKED','25% OPEN · 75% LOCKED','상세 분석 75% 통제 공개']), '무료 가치 제공과 상세 산출물 잠금의 명확한 분리');
add('리스크 시각화 품질', 8, hasAll(js + css, ['02 · TRUST EXPOSURE MAP','.vrd-heatmap','.vrd-heat-track','LOCKED EXPOSURE AREAS','문제 신호','관련 요소 범위']), '영역별 상대 강도·관련 범위·잠금 영역');
add('구매 여정 마찰 분석', 8, hasAll(js, ['03 · BUYER FRICTION PATH','첫 방문','상품 검토','결제 직전','결정 또는 이탈','실제 이탈률 측정값이 아니라']), '고객 여정과 결제 직전 마찰 지점');
...
add('샘플 화면과 실제 렌더러 일치', 2, hasAll(html, ['REPORT ID','FREE PREVIEW · CONTROLLED DISCLOSURE','Executive Decision','Trust Exposure Map','Evidence Ledger 잠금','Fix Specification 잠금']), '실행 전 샘플도 전문 보고서 체계 반영');
...
const report = { ok: score === 100 && tests.every(test => test.pass), contract: 'veridion-2.3-report-excellence-scorecard', standard: 'top-0.5-percent-professional-report-target', checkedAt: new Date().toISOString(), score, maximum: 100, required: 100, failed: tests.filter(test => !test.pass), tests };
