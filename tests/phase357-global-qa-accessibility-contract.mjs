...
for (const file of ['docker-compose.yml', 'deploy/docker-compose.coolify.yml']) {
  add(`${file}:phase357-operational-forwarding`, () => {
...
const report = { ok: failures.length === 0, contract: 'phase357-global-qa-accessibility-contract', checkedAt: new Date().toISOString(), checked: checks.length, failed: failures.length, failures, checks };
