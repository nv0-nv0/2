...
상세 변경 보고서는 `docs/current/PHASE258_STRUCTURAL_HARDENING_REPORT_20260514_KO.md`를 확인하면 된다.
...
- 유료 서비스: 무료 데모에서 잠긴 항목의 전체 내용을 누락 없이 공개하도록 `paidFullDetailContract`를 생성합니다. 각 항목은 근거, 출처 URL, 한계, 권장 조치, 수정 문구, 수용 기준을 포함합니다.
...
단, 실제 운영 결제 승인, SMTP, 외부 저장소, 외부 결제 승인/SMTP/저장소 연동은 운영 키와 인프라 값이 필요합니다. 패키지 내부에서는 외부 키 없이 가능한 데모 표시, 유료 상세 계약, 맞춤 운영 문서 생성, UI 렌더링, 테스트 게이트를 검증합니다.
...
- 유료 상세 리포트: 무료에서 잠긴 전체 근거, 문제 위치, 한계, 수정 문구, 적용 위치, 재검사 기준을 누락 없이 공개합니다.
